...
/* Local variables.  */
__private const struct base_table_t _itoa_base_table[] =
{
...
__private const char _itoa_upper_digits[36] = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
__private const char _itoa_lower_digits[36] = "0123456789abcdefghijklmnopqrstuvwxyz";

UNSAFE_BEGIN("-Warray-bounds");
...
unsigned itoa(char* buf, size_t size, unsigned base, int upper, unsigned long long value){
	char* stconv = _itoa( value, buf+size, base, upper);
	dbg_info("_itoa(%d, %llu) = %s[+%lu](%lu)", base, value, stconv, (uintptr_t)stconv - (uintptr_t)buf, (buf+size)-stconv);

	size_t len = (buf+size) - stconv;
	iassert(len + 1 < size );
	if( stconv != buf ) memmove(buf, stconv, len);
	buf[len] = 0;
	return len;
}
